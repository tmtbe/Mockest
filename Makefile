...
test.sandbox.record:
	...
	docker run --platform=linux/arm64/v8 -d --network mockest --name collector -v ${PWD}/replay:/home  tmtbe/mockest-collector:master
	docker run --platform=linux/arm64/v8 -d --cap-add=NET_ADMIN --cap-add=NET_RAW  --network mockest --name proxy tmtbe/mockest-proxy:master all
	docker run --platform=linux/arm64/v8 -d --network=container:proxy --name demo tmtbe/mockest-demo:master
	docker run --platform=linux/arm64/v8 -d --network mockest --name outbound-demo tmtbe/mockest-demo:master
test.sandbox.replay:
	docker network create mockest
